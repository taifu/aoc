...
class Solution:
    ...
    def compute(self, connections: Dict[Tuple[str, Any, str], str], values: Dict[str, int]) -> int:
        values = values.copy()
    ...
    def wrong(self) -> str:
        ...
        for n in range(wires):
            ...
            values[lab_x] = 1
            calc = self.compute(self.connections, values)
            if calc is None:
                return None
            if calc != 2 ** n:
                wrong.add(n)
    ...
    def count(self) -> str:
        return self.compute(self.connections, self.values)
    ...
    def count2(self) -> str:
        ...
        for swaps in combinations(improving, 4):
            if len(set(wire for wires in swaps for wire in wires)) < 8:
                continue
            labels = []
